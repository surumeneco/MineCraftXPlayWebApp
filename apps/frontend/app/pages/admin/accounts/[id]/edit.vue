  ...
  <section>
    ...
    <template v-else>
      ...
      <form class="d-flex gap-2 flex-wrap mb-4" @submit.prevent="addDiscord">
        ...
        <button type="submit" class="btn btn-primary" :disabled="busy || !discordId">Discord IDを追加</button>
      </form>
      ...
      <div class="d-flex gap-2 flex-wrap mt-4">
        ...
        <NuxtLink v-if="!account.is_protected" :to="`/admin/accounts/merge?source=${account.id}`" class="btn btn-warning">統合元として選択</NuxtLink>
...
const account = ref<AccountRecord | null>(null), discordId = ref(''), minecraftName = ref('')
...
async function load() { account.value = await get<AccountRecord>(`/admin/accounts/${id.value}`) }
