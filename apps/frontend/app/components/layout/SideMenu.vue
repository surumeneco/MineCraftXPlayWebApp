  ...
  <aside aria-label="サイドメニュー">
    <UiPanel>
      <div class="d-flex justify-content-between align-items-center gap-2 mb-3">
        ...
        <NuxtLink to="/info/notice" class="small flex-shrink-0">一覧</NuxtLink>
      </div>
      <UiNote v-if="status === 'pending' || status === 'idle'" role="status">読み込み中…</UiNote>
      <UiNote v-else-if="error">お知らせを取得できませんでした。</UiNote>
      <UiNote v-else-if="latest.length === 0">公開中のお知らせはありません。</UiNote>
