...
suite('populated legacy database account migration', () => {
  it('preserves administrators, sessions, images, notices and adds reversible-merge metadata', async () => {
    ...
    try {
      ...
      await sql.begin(async tx => {
        ...
        for (const filename of ['001_notices.sql', '002_accounts.sql', '003_account_profiles.sql', '004_reversible_account_merges.sql']) {
        ...
        const identity = await tx`SELECT account_id, username, display_name, merge_origin FROM account_discord_identities WHERE discord_id=${discordId}`
        expect(identity).toHaveLength(1)
        expect(identity[0].username).toBe(discordId)
        expect(identity[0].display_name).toBe(discordId)
        expect(identity[0].merge_origin).toBeNull()
        ...
        const image = await tx`SELECT merge_origin FROM images WHERE uploaded_by=${accountId}`
        expect(image).toHaveLength(1)
        expect(image[0].merge_origin).toBeNull()
        ...
        expect((await tx`SELECT to_regclass('account_merges') AS present`)[0].present).not.toBeNull()
