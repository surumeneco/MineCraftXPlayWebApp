...
- 下書き・公開取り消し済みの記事を公開するたびに、参加者向けのお知らせ専用チャンネルへ `新しいお知らせが投稿されました: <タイトル> 🏷️<タグ名を半角空白区切り>` を送信し、次の行に詳細URLを追加する。
- 既に公開中の記事の `body_delta`（JSONB）の内容が変更された場合のみ `お知らせの内容が更新されました: <タイトル> 🏷️<タグ名を半角空白区切り>` を送信し、次の行に詳細URLを追加する。タグのみの編集、同値のDelta、下書き・非公開記事の編集は通知しない。
